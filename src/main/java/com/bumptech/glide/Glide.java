...
public class Glide implements ComponentCallbacks2 {
    ...
    private static void initializeGlide(@NonNull Context context, @NonNull GlideBuilder glideBuilder) {
        ...
        if (annotationGeneratedGlideModules == null || annotationGeneratedGlideModules.isManifestParsingEnabled()) {
            emptyList = new ManifestParser(origApplicationContext).parse();
        }
            ...
            while (it.hasNext()) {
                ...
                if (excludedModuleClasses.contains(glideModule.getClass())) {
                    if (Log.isLoggable(TAG, 3)) {
                        Log.d(TAG, "AppGlideModule excludes manifest GlideModule: " + glideModule);
                    }
                    it.remove();
                }
            }
        ...
        if (Log.isLoggable(TAG, 3)) {
            for (GlideModule glideModule2 : emptyList) {
                Log.d(TAG, "Discovered GlideModule from manifest: " + glideModule2.getClass());
            }
        }
        ...
        for (GlideModule applyOptions : emptyList) {
            applyOptions.applyOptions(origApplicationContext, glideBuilder);
        }
        if (annotationGeneratedGlideModules != null) {
            annotationGeneratedGlideModules.applyOptions(origApplicationContext, glideBuilder);
        }
        Glide build = glideBuilder.build(origApplicationContext);
        for (GlideModule registerComponents : emptyList) {
            registerComponents.registerComponents(origApplicationContext, build, build.registry);
        }
        if (annotationGeneratedGlideModules != null) {
            annotationGeneratedGlideModules.registerComponents(origApplicationContext, build, build.registry);
        }
        origApplicationContext.registerComponentCallbacks(build);
        glide = build;
    }
