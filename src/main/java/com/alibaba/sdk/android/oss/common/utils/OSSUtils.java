    ...
    public static void signRequest(RequestMessage requestMessage) throws IOException {
        ...
        if (requestMessage.isAuthorizationRequired()) {
            if (requestMessage.getCredentialProvider() != null) {
                ...
                String format = String.format("%s\n%s\n%s\n%s\n%s%s", new Object[]{httpMethod, str, str2, str3, sb2, buildCanonicalizedResource(requestMessage.getBucketName(), requestMessage.getObjectKey(), requestMessage.getParameters())});
                String str4 = "---initValue---";
                if (z || (credentialProvider instanceof OSSStsTokenCredentialProvider)) {
                    str4 = sign(oSSFederationToken.getTempAK(), oSSFederationToken.getTempSK(), format);
                    ...
                    str4 = sign(oSSPlainTextAKSKCredentialProvider.getAccessKeyId(), oSSPlainTextAKSKCredentialProvider.getAccessKeySecret(), format);
                } else if (credentialProvider instanceof OSSCustomSignerCredentialProvider) {
                    str4 = ((OSSCustomSignerCredentialProvider) credentialProvider).signContent(format);
                }
